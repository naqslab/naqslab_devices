    ...
    def generate_code(self, hdf5_file):
        ...
        for output in self.child_devices:
            try:
                ...
                Format must be \'channel n\' with n equal to one of {}.'''
        ...
        for connection in DDSs:
            dds = DDSs[connection]
            _ = dds.frequency.get_change_times()
            dds.frequency.make_timeseries([])
            dds.frequency.expand_timeseries()

            _ = dds.amplitude.get_change_times()
        ...
        # Get channels to program
        stat_DDSs = set(DDSs) & set(self.allowed_chans)
        ...
        static_table = np.zeros(1,dtype=static_dtypes) # static outputs only have a single value

        for connection in DDSs:
            dds = DDSs[connection]
            static_table[f'freq{connection}'].fill(1)
            static_table[f'freq{connection}'] = dds.frequency.raw_output[0]
            static_table[f'amp{connection}'].fill(1)
            static_table[f'amp{connection}'] = dds.amplitude.raw_output[0]
            static_table[f'gate{connection}'].fill(0)
            static_table[f'gate{connection}'] = connection in self.enabled_chans # returns True if channel 0 enabled
