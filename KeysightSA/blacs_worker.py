#####################################################################
#                                                                   #
# /naqslab_devices/KeysightSA/blacs_worker.py                       #
#                                                                   #
...
class KeysightSAWorker(VISAWorker):   
    # define instrument specific read and write strings
    setup_string = '*ESE 60;*SRE 32;:FORM:TRAC:DATA REAL,64'
    # *ESE does not disable bits in ESR, just their reporting to STB
    # need to use our own mask
    esr_mask = 60
    # note that different acquisition types need different commands
    read_sweep_parameters_string = ':SENS:BWID:RES?;VID?;:FREQ:SPAN?;START?;STOP?;:SWE:TIME?;:AVER:COUN?'
    read_trace_string = ':TRAC:DATA? TRACE{0:d}'
    model_ident = 'N9020A'

    def read_trace(self,trace_num):
        '''Reads the trace corresponding to trace_num'''
        res = self.connection.query_binary_values(self.read_trace_string.format(trace_num),
                                                    datatype='d',is_big_endian=True,container=np.array)
        return res

    def read_trace_params(self):
        '''Reads current sweep parameters.'''
        [RBW, VBW, fSpan, fStart, fStop, acqTime, aveN] = self.connection.query_ascii_values(self.read_sweep_parameters_string,separator=';')
        return_dict = {'RBW':RBW,'VBW':VBW,'fSpan':fSpan,'fStart':fStart,'fStop':fStop,'acqTime':acqTime,'aveN':aveN}
        return return_dict
    ...
    def init(self):
        ...
        if ('Keysight' in ident_string or 'Agilent' in ident_string) and (self.model_ident in ident_string):
            # Supported!
        else:
